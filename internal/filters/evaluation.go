...
func (te TraceEvaluator) EvalScenario() (*string, error) {
	return te.evalFilter(te.scenario.Filter)
}
...
func (te TraceEvaluator) evalFilter(f model.Filter) (*string, error) {
	...
	if f.Type == model.WORKLOAD {
		...
		for workloadId, sets := range matchingSets {
			err := te.traceStore.NewUnionSet(workloadId, sets...)
			if err != nil {
				return nil, err
			}
		}
	...
	resultSetName := uniqueStringFromStringSet(workloadTraceSetNames)
...
func uniqueStringFromStringSet(set []string) string {
